...
| [terrariabonker](terrariabonker/) | From-scratch live-memory trainer and item editor for Terraria (Windows build under Proton/wine-mono). Locates the player and inventory in `/proc/<pid>/mem` with no hardcoded addresses, then freezes HP/mana (godmode), edits stats, and edits inventory items (type, stack, damage, auto-attack, mining/placement speed, reach) from a CLI and a PyQt6 control panel. Documents the persistent-vs-frame-reset boundary that separates external memory editing from Cheat-Engine code patching. |
