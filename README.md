...
| Project | Description |
| :--- | :--- |
| [audio-source-switcher](audio-source-switcher/) | **Superseded by [ototo](https://github.com/ushineko/ototo)**, its Go port: one binary, no Python. Kept here as it was: a PyQt6 audio manager with priority auto-switching, mic association, Bluetooth device management, JamesDSP integration, global hotkeys, and a custom on-screen volume indicator (OSD). Still a readable example of PipeWire/PulseAudio graph manipulation and KDE Wayland window rules. |
...
| [audio-source-switcher](audio-source-switcher/) | Superseded by [ototo](https://github.com/ushineko/ototo). Audio manager with priority auto-switching, mic association, Bluetooth device management, JamesDSP integration, global hotkeys, and a custom on-screen volume indicator (OSD). |
...
| [claude-usage-widget-windows](claude-usage-widget-windows/) | Cross-platform (macOS + Windows) PySide6 floating widget and menu-bar/tray icon displaying Claude Code usage from the Anthropic OAuth API. Color-coded progress bar, countdown timer, model breakdowns, selectable font size; macOS `.app` reads credentials from the login Keychain. Also a Qt-free terminal mode (`--tui` full-width dashboard / `--line` one-shot) for tmux/herd helper panes, with a cooperative cross-process cache so many panes share ~1 API request per interval. Autodetects every configured Claude login and reports one line per account, labeled by profile and plan type. |
...
| [peripheral-battery-monitor](peripheral-battery-monitor/) | Compact always-on-top dashboard for Logitech, Keychron, Arctis, and AirPods battery monitoring, plus configurable real-time/cumulative network bandwidth with Tailscale exit-node awareness and AIO liquid-cooler thermals plus RGB control via OpenLinkHub. |
...
| [vscode-gather](vscode-gather/) | Gathers all VS Code windows onto a single monitor and maximizes them. Cross-platform: KWin scripting via D-Bus on KDE Plasma 6 / Wayland (Linux), AppleScript via `osascript` on macOS (requires Accessibility access). |
