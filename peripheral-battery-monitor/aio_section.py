...
**Reads only.** Every write to this cooler and to every lit device belongs to
hotaru: its RGB, its LCD, the scenes and the hotkeys that applied them left
here in one commit, because two processes writing one hidraw node is the thing
this stack has been most careful about. See spec 039.

What is left is the poll, the rows, the sparkline and the pump-failure alert —
the alert because it is owned where it is visible, and this is the window
somebody actually looks at. The reads continue until this program is rewritten
in Go and becomes a consumer of hotaru's API, at which point it stops touching
the cooler at all.

`aio_queue.LiquidctlQueue` stays, now serialising only the status poll against
itself. See the AIO runbook in ~/git/sysadmin/runbooks/.
"""
...
# liquidctl runs out of process (spec 020), driven through the queue rather than
# subprocess for the same reason the HTTP calls use QNetworkAccessManager: a
# blocking call on the GUI thread freezes the widget. Measured at ~0.16 s on
...
class AioSection(QFrame):
    ...
    def __init__(self, initial_settings: dict | None = None, parent=None):
        ...
        # One queue for the status poll. It serialised every liquidctl
        # invocation on this device when there were writes as well; there are
        # not, and it stays because two reads racing each other on one hidraw
        # node is still worth preventing.
    ...
    def _request_liquidctl(self):
        ...
        Spec 020 ran this as its own QProcess. It goes through `LiquidctlQueue`
        instead, which serialised it against the RGB and LCD writes that used
        to live here and now keeps two reads from overlapping on one hidraw
        node.
