...
Qt-free, and free of any project-local import, so this file is copied verbatim
into ``peripheral-battery-monitor`` (the arrangement ``usage_shape.py`` and
``accounts.py`` already use). The two projects therefore share one cache
directory and one gate per account: the GUI widget, every ``--tui`` pane and
any one-shot ``--line`` call together make ~1 API request per account per
window, instead of each polling on its own.

The fetch function is injected rather than imported, because the two projects
implement it in different places.
"""
...
# The default fetcher, when this module is used from inside the packaged
# project. The mirrored copy has no such sibling and passes ``fetch=`` instead.
try:                                    # pragma: no cover - layout dependent
    from .oauth import fetch_claude_usage as _default_fetch
except ImportError:                     # pragma: no cover - flat layout
    _default_fetch = None


def get_cache_dir() -> Path:
    """Cache directory shared by every process reading Claude usage.

    Defined here rather than in a project config module so both projects
    resolve the identical path and genuinely share the cache.

    macOS: ``~/Library/Caches/claude-usage-widget``.
...
def fetch_usage_cached(
    ...
    ``force`` skips the freshness gate for an explicit user-initiated refresh.
    It still takes the lock and still writes the result, so a manual refresh
    never stampedes and always benefits every other reader. It deliberately
    does not skip a *backoff* gate — see below.
    """
